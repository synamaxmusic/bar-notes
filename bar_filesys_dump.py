## Beetle Adventure Racing! Filesys Dump by SynaMax
##
## Based on previous work on Pilotwings 64 by queueRAM
## https://github.com/queueRAM/pilotwings_64/blob/master/pw64_filesys_dump.py
...
def pw64_dump_filesys(fname, startOffset, hexSize):
    ...
    with open(fname, 'rb') as fin:
        ...
        while (dumping):
            ...
            magic2Int = int.from_bytes(magic2, byteorder='big')
            #breakpoint()
            if magicInt == 0:
                print('00000000 [EOF]')
                break
            ...
            if magicStr == 'FORM':
                ...
                while (fin.tell() < formEnd):
                    ...
                    ### UV = "UltraVision" ###
                    #
                    # 0x237D0 = First FORM table contains the pointers to all the
                    # FORM chunks in the ROM.  The pointers are offset by
                    # the start of the NEXT 'FORM' header (0x25FD0).
                    #
                    # You can think of FORM #0 as a table of contents, while FORM #1
                    # is the start of actual game data.
                    #
                    # Weird 'UVFT' Header:
                    #
                    #   For FORM #0 there's a UVFT header that's immediately followed
                    #   by 'UVMO'. I'm not sure why this is because 'UVFT' is related
                    #   to fonts.
                    #   Regardless, these headers show that all the FORM chunks are
                    #   arranged by their type.  These types are:
                    #
                    ### Code-related ###
                    #
                    # 'UVMO' = Modules
                    #   'MODU' containers inside, which appear to have
                    #   'CODE' chunks embedded.
                    #
                    # 'UVTT' = Track
                    #   Has 'CODE', 'PNTS', 'MDBG', 'RELA'
                    #
                    # 'UVDS' = ??
                    #   Has 'CODE'...could be debug or "dynamic set" related?
                    #
                    # 'UVTX' = Texture
                    #   Presumably has MIO0 compressed texture data
                    #
                    # 'UVTR' = Terrain  
                    #
                    # 'UVCT' = Contour
                    ...
                    # 'UVEN' = Environment
                    #
                    # 'UVRW' = Save Game? Referenced as "ufile" (user file?) 
                    #
                    # 'UVPX' = Particle Effects
                    #
                    # 'UVBT' = Blit??
                    #   
                    ...
                    #
                    # 'UVSX' = Sound FX
                    # 'UVMS' = Music Sequences
                    #- 'SEQS' = Music MIDI Sequences
                    #
                    # 'UVMB' = Music Banks
                    ...
                    ## Beetle Adventure Racing has two types of UVFT?! One DOES have length, while the other doesn't
                    if magicStr in ['UVTT', 'UVDS', 'UVTX', 'UVTR',
                                    'UVCT', 'UVVL', 'UVMD', 'UVAN',
                                    'UVEN', 'UVRW', 'UVPX', 'UVBT',
                                    'UVSX', 'UVMS', 'UVMB', 'UVTS',
                                    'COMM', 'CODE', 'MDBG', 'RELA',
                        ...
                        # check for 'PAD '
                        if length == 1346454560: # This is 'PAD ' as an integer
                            padLength = int.from_bytes(fin.read(4), byteorder='big')
                            sectionData = fin.read(padLength)
                            print('  %s: 0x%06X:' % (magicStr + 'PAD ', padLength))
                            hexdump(sectionData)
                            fileOffset = fin.tell() - 8
                        else:
                        ...
                        blockType = magicStr
                        uvftSize = fin.read(4)
                        uvftSizeInt = int.from_bytes(uvftSize, byteorder='big')
                        uvftStr = uvftSize.decode('utf-8')
                        if uvftSizeInt <= 52:
                            length = uvftSizeInt
                            sectionData = fin.read(length)
                            print('  %s: 0x%06X:' % (magicStr, length))
                            ...
                            length = int.from_bytes(fin.read(4), byteorder='big')
                            sectionData = fin.read(length)
                            print('  %s: 0x%06X:' % (magicStr + uvftStr, length))
                            hexdump(sectionData)
                            #breakpoint()
                            fileOffset = fin.tell() - 4
                        elif uvftStr == 'PAD ':
                            length = int.from_bytes(fin.read(4), byteorder='big')
                            sectionData = fin.read(length)
                            print('  %s: 0x%06X:' % (magicStr + uvftStr, length))
                        ...
                        padcheck = fin.read(4)
                        padStr = padcheck.decode('utf-8')
                        if padStr == 'PAD ':
                            length = int.from_bytes(fin.read(4), byteorder='big')
                            sectionData = fin.read(length)
                            print('  %s: 0x%06X:' % (magicStr + padStr, length))
                            hexdump(sectionData)
